...
int scopeTable::hashfunction2(string str)
{
  int  hash = 0;
 int i = 0;
   int len = str.length();
...
bool scopeTable::insert(symbolInfo &newsymbol, ostream &output)
{
    ...
    {
        output << "" << newsymbol.getname() << " "<<newsymbol.gettype()<<endl<<"\t'"<<newsymbol.getname()<<"' already exists in the current ScopeTable" << endl;
    ...
    output 
           << "\tInserted in ScopeTable# " << id << " at position " << newpos+1 << ", " << chain << endl;
...
bool symbolTable::exitScope(ofstream &output,int saffat=0)
{
  ...
  currentScope=NULL;
        ...
        output<<"\tScopeTable# 1 cannot be removed"<<endl;
        }
        return false;
...
int main()
{
    ifstream filei("input1.txt");
    ofstream fileo("output1.txt");
    string str, inp1, name, type, line;
    ...
    if (filei.is_open())
    {
        ...
        iss1 >> buckets;
        cout << "size" << buckets << endl;
        symbolTable stable(buckets, fileo);
        cout << "After ctor, stable.buckets: " << stable.getbuckets() << endl;
        cout << "After ctor, scopetable.buckets: " << stable.getCurrentScope()->getbuckets() << endl;
        while (true)
        {
            getline(filei, line);
            cout << line << endl;
            ...
            if (inp1 == "I")
            {
                if ((iss >> name >> type))
                {
                      cout << "name and type = " << name << " ," << type;
                    symbolInfo* newsym=new symbolInfo(name,type);
                    fileo<<"Cmd " << cmd << ": I ";
                    bool nabid = stable.insert(*newsym, fileo);
                    cout << nabid << " " << endl;
                ...
                else
                {
                    ...
                    else
                    {
                     ...
                     fileo<<"\tNumber of parameters mismatch for the command I"<<endl;
            ...
            else if (inp1 == "L")
            {
                if (iss >> name>>type)
                {
                     fileo<< "Cmd " << cmd << ": L "<<name<<" "<<type<<endl;
                     fileo<<"\tNumber of parameters mismatch for the command L"<<endl;
                    cout << "Error" << endl;
                }
                else
                {
                    fileo<< "Cmd " << cmd << ": L ";
                    cout<< "Cmd " << cmd << ": L "<<name<<type<<"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"<<endl;
                    stable.lookup(name, fileo);
                }
            }
            else if (inp1 == "D")
            {
                if (iss >> name)
                {
                    ...
                    fileo<< "Cmd " << cmd << ": D ";
                    stable.remove(name, fileo);
                }
                else
                {
                    fileo<< "Cmd " << cmd << ": D ";
                    if(type!="q")
                    fileo<<name<<" "<<type<<endl;
                    if(name=="q" && type=="q")
                    fileo<<" "<<endl;
                     fileo<<"\tNumber of parameters mismatch for the command D"<<endl;
                }
            }
            else if (inp1 == "P")
            {
                ...
                else
                {
                    ...
                    {
                        fileo<< "Cmd " << cmd << ": ";
                        stable.printCurrent(fileo);
                    }
                    else
                    {
                        fileo<< "Cmd " << cmd << ": ";
                        stable.printAll(fileo);
                    }
                }
            }
            else if (inp1 == "S")
            {
                fileo << "Cmd " << cmd << ": S" << endl;
                stable.enterScope(fileo);
            }
            else if (inp1 == "E")
            {
                fileo << "Cmd " << cmd << ": E" << endl;
                stable.exitScope(fileo);
            }
            else
            {
                  fileo << "Cmd " << cmd << ": Q" << endl;
                while (stable.exitScope(fileo,1)==true)
                {
