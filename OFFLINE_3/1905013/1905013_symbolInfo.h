...
symbolInfo* lookup(string value, ostream &output,int w=0)
{
...
bool insert(symbolInfo &newsymbol, ostream &output)
{
...
bool Delete(string newsymbol, ostream &output)
{
...
public:
   symbolTable(int bucket_size, ostream &output)
{
...
bool remove(string newsymbol, ostream &output)
{
    bool tmp = currentScope->Delete(newsymbol, output);
    return tmp;
}
symbolInfo* lookup(string key, ostream &output)
{
...
symbolInfo* lookuphere(string key,ofstream &output)
{
    scopeTable *tmp = currentScope;
    symbolInfo *ans = NULL;
    //cout << "from symboltable key = " << key << endl;
